...
- Header: `GeneralsMD/Code/GameEngine/Include/GameLogic/Locomotor.h` – `LocomotorTemplate`, enums
...
- INI: Inside an `Object` template, use `Locomotor = <LocomotorName>` via AI/update parsing. The `LocomotorTemplateDefinition` is defined at root:
  - `LocomotorTemplate <Name> ... End`
...
### Helicopter Hover
```
LocomotorTemplate HelicopterHover
  Surfaces                  = AIR
  Appearance                = HOVER
  Speed                     = 80
  TurnRate                  = 2.5
