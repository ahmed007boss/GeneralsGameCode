...
void WeaponTemplate::postProcessLoad()
{
	...
	// TheSuperHackers @feature author 01/01/2025 Set automatic DirectHitSideOverride based on damage type
	if (m_directHitSideOverride == HIT_SIDE_UNKNOWN)
	{
		switch (m_damageType)
		{
			case DAMAGE_LAND_MINE:
				m_directHitSideOverride = HIT_SIDE_BOTTOM;
				break;
			case DAMAGE_JET_MISSILES:
			case DAMAGE_STEALTHJET_MISSILES:
				m_directHitSideOverride = HIT_SIDE_TOP;
				break;
			default:
				// Keep as HIT_SIDE_UNKNOWN for other damage types
				break;
		}
	}
...
//DEBUG_LOG(("WeaponTemplate::dealDamageInternal: dealing damage %s at frame %d",m_name.str(),TheGameLogic->getFrame()));
	...
	if (getProjectileTemplate() == NULL || isProjectileDetonation)
	{
		...
		for (; curVictim != NULL; curVictim = iter ? iter->nextWithNumeric(&curVictimDistSqr) : NULL)
		{
			...
			HitSide hitSide = HIT_SIDE_UNKNOWN;
			if (curVictim && source)
			{
				// Calculate distance between shell and victim
				Real distance = sqrt(curVictimDistSqr);
				// Get collision size of the victim object
				Real victimCollisionSize = curVictim->getGeometryInfo().getBoundingSphereRadius() * 1.1f;
				// Determine if this is a direct hit
				Bool isDirectHit = (distance <= victimCollisionSize);
				
				// Get the actual attacker (not the projectile/shell)
				Object* attacker = source;
				ObjectID producerID = source->getProducerID();
				if (producerID != INVALID_ID)
				{
					...
					if (producer)
					{
						if (isDirectHit)
						{
							// Use original attacker if direct hit
							attacker = producer;
						}
						else
						{
							// Use shell as attacker if indirect hit
							attacker = source;
						}
					}
				}

				// Check if weapon has hit side override (for jets, mines, etc.)
				
				if (curVictimDistSqr <= primaryRadiusSqr)
				{
					// Primary damage
					if (isDirectHit && getDirectHitSideOverride() != HIT_SIDE_UNKNOWN)
					{
						hitSide = getDirectHitSideOverride();
					}
					else if (getPrimaryHitSideOverride() != HIT_SIDE_UNKNOWN)
					{
						hitSide = getPrimaryHitSideOverride();
					}
					else
					{
						hitSide = HitSideDetection::determineHitSide(curVictim, attacker);
					}
				}
				else
				{
					// Secondary damage (radius damage)
					if (isDirectHit && getDirectHitSideOverride() != HIT_SIDE_UNKNOWN)
					{
						hitSide = getDirectHitSideOverride();
					}
					else if (getSecondaryHitSideOverride() != HIT_SIDE_UNKNOWN)
					{
						hitSide = getSecondaryHitSideOverride();
					}
					else
					{
						hitSide = HitSideDetection::determineHitSide(curVictim, attacker);
			...
			damageInfo.in.m_sourcePlayerMask = 0;
			damageInfo.in.m_damageStatusType = damageStatusType;
			damageInfo.in.m_hitSide = hitSide;
